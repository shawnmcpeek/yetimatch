...
actual fun loadResourceBytes(path: String): ByteArray? {
    ...
    val dot = filename.lastIndexOf('.')
    val name = if (dot >= 0) filename.substring(0, dot) else filename
    val ext = if (dot >= 0) filename.substring(dot + 1) else "png"
    val resPath = NSBundle.mainBundle.pathForResource(name, ext, subpath) ?: return null
    val nsData = NSData.dataWithContentsOfFile(resPath) ?: return null
